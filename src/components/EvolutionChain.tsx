      ...
      .catch((error: unknown) => {
        if (!obsolete) {
          setState({
            status: "error",
            message:
              error instanceof Error ? error.message : "Evolution indisponible",
          });
        }
      });
  ...
  return (
    <section aria-label="Famille d'évolution">
      ...
      {state.status === "success" &&
        (state.chain ? (
          <>
            {state.chain.evolves_to.length === 0 && (
              <p>Cette famille n'a pas d'évolution</p>
            )}
          ...
          <p>Aucune chaine d'Ã©volution disponible.</p>
        ))}
    </section>
  );
}
